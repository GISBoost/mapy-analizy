...
Interaktywna mapa: gdzie w 6 miastach (Łódź, Gdańsk, Warszawa, Poznań, Kraków, Szczecin)
realne opóźnienia transportu publicznego najbardziej psują dostępność do szkół, aptek,
uczelni i centrów handlowych — siatka heksagonalna (250 m i 500 m; Warszawa tylko 500 m),
dostępność 30 min w oknie 7:00–9:00, porównanie rozkładu **statycznego** GTFS vs
**zrealizowanego P50** (median tego, co pojazdy faktycznie zrobiły), ten sam dzień
(Łódź 2026-08-21, pozostałe 2026-08-24).
...
`data/hex_<miasto>_<res>.geojson` — heksagony z polami `pop_total`,
...
`data/manifest.json` — lista miast, każde z granicami mapy i dostępnymi rozdzielczościami;
etykiety kategorii.
`data/boundary_<miasto>.geojson` (obwódka miasta) i
`data/siatka_<miasto>_<res>.geojson` (sama siatka heksagonalna, bez atrybutów) —
...
Wygenerowane z [`easy-R5/tools/realtime_delay_cities`](https://github.com/GISBoost/easy-R5/tree/main/tools/realtime_delay_cities)
(`export_geojson.py`; Łódź nadal z siostrzanego `realtime_delay_lodz`) — tam jest pełna
metoda, w tym dlaczego `base_<kategoria> = 0` oznacza "nie ma czego tracić ani zyskiwać",
a nie "bez zmian". Odświeżenie danych = ponowne uruchomienie pipeline'u tam i re-eksport
do `data/` (na razie ręcznie).

## Trzy osie przełączania

- **Wybór miasta u góry** — przeładowuje granice, dane i dopasowuje widok mapy.
- **Zakładki u góry = rozdzielczość siatki** (250 m / 500 m; Warszawa tylko 500 m) — przeładowuje dane.
